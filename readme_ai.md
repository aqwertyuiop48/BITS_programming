<h3> Browser </h3>


<h3> CLI / IDE extension </h3>

- Codex: https://marketplace.visualstudio.com/items?itemName=openai.chatgpt
- Cline : https://marketplace.visualstudio.com/items?itemName=saoudrizwan.claude-dev
- Github copilot: https://marketplace.visualstudio.com/items?itemName=GitHub.copilot-chat
- Gemini (This client is no longer supported for Gemini Code Assist for individuals) : https://marketplace.visualstudio.com/items?itemName=Google.geminicodeassist
- Anthropic (no free tier) : https://marketplace.visualstudio.com/items?itemName=RizwanAnsari.anthropic-coder
- Openrouter:
  
<code>
curl https://openrouter.ai/api/v1/chat/completions \  -H "Content-Type: application/json" \  -H "Authorization: Bearer sk-....." \  -d '{  "model": "meta-llama/llama-3.1-8b-instruct",  "messages": [    {      "role": "user",      "content": "What is the meaning of this?"    }  ]}'
</code>

- Roo code (not loading properly) : https://marketplace.visualstudio.com/items?itemName=RooVeterinaryInc.roo-cline


<hr>
- Ollama:
    -   https://ollama.com/download/OllamaSetup.exe
    -   ollama launch claude --model minimax-m3:cloud
    -   To use other models:
        -   ollama pull qwen2.5-coder:7b
        -   ollama run qwen2.5-coder:7b "say hi"

</hr>


To view all Google cloud projects:
- https://console.cloud.google.com/projectselector2/billing
